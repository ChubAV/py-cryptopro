...
window.onload = () => {
  ...
  document.getElementById("client1").addEventListener("click", (event) => {
    ...
    oFReader.onload = function (oFREvent) {
      cadesplugin.async_spawn(function* (args) {
        ...
        SignCreate(thumbprint_cert, sBase64Data, format_sign.value).then(
          function (signedMessage) {

            
            if (format_sign.value == 0) {
              var blob = base64ToFile(signedMessage, "myFile.p7s");
              const fileInput = document.getElementById("fileout");
              const myFile = new File([blob], "myFile.p7s");
  ...
  document.getElementById("format-sign").addEventListener("change", (event) => {
    setFormatSing()
  })
